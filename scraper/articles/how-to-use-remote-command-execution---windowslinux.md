...
- [Controlling Remote Command Received by the Device](#Controlling) 
    **NOTE**   In order to use this feature, you will need a **Pro Plus** **or above plan**.    The Execute Remote Commands feature allows you to remotely manage screens running on Windows/Linux/Raspberry Pi. It allows command to be pushed remotely to individual devices or groups of devices simultaneously.
...
-  **Tags** - Utilizing tags, you can execute the command on a group of devices. In the below example, the command will be submitted to all devices tagged as Windows or Raspberry Pi. 
 ...
 After a command is executed, it will appear in section 3: the command history.
    ...
    **NOTE**   The **Refresh & Relaunch** command can be used on any device. The **Reboot Device **command cannot be used on certain Android devices.    
---
